...
void engine::GpuHDRI2CubemapPass::initInner()
{
	bInitPipeline = false;
	createPipeline();
...
void engine::GpuHDRI2CubemapPass::record(uint32 backBufferIndex)
{
...
void engine::GpuHDRI2CubemapPass::createPipeline()
{
	...
	for(uint32 index = 0; index < backBufferCount; index++)
	{
		...
		filterImage.imageView = m_renderScene->getSceneTextures().getEnvCube()->getImageView();;
	...
	for(uint32 index = 0; index<backBufferCount; index++)
	{
		...
		auto* shaderModule = VulkanRHI::get()->getShader("media/shader/fallback/bin/hdri2cubemap.comp.spv");
...
void engine::GpuHDRI2CubemapPass::destroyPipeline()
{
	if(!bInitPipeline) return;
