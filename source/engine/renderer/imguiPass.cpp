...
void setupStyle()
{
	...
	colors[ImGuiCol_Border]                 = ImVec4(0.09f, 0.09f, 0.23f, 0.50f);
...
void ImguiPass::initImgui()
{
	...
	const auto font_size    = 13.0f;
	const auto font_scale   = 1.0f;
	io.Fonts->AddFontFromFileTTF((s_fontsDir + std::string("consola.ttf")).c_str(), font_size, NULL, io.Fonts->GetGlyphRangesJapanese());
